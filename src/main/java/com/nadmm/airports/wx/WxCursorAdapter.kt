...
class WxCursorAdapter(context: Context?, c: Cursor?, private val mDelegate: WxDelegate) :
    ...
    private fun getViewHolder(view: View): ViewHolder {
        ...
        if (holder == null) {
            val binder = WxListItemBinding.bind(view)
            holder = ViewHolder()
            holder.stationName = binder.wxStationName
            holder.stationId = binder.wxStationId
            holder.stationInfo = binder.wxStationInfo
            holder.stationInfo2 = binder.wxStationInfo2
            holder.stationFreq = binder.wxStationFreq
            holder.stationPhone = binder.wxStationPhone
            holder.stationWx = binder.wxStationWx
            holder.stationWx2 = binder.wxStationWx2
            holder.reportAge = binder.wxReportAge
