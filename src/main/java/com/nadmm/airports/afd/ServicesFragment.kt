...
class ServicesFragment : FragmentBase() {
    ...
    private fun showDetails(result: Array<Cursor?>) {
        ...
        try {
            ...
            UiUtils.showToast(requireContext(),"Unexpected error occurred: ${e.message}")
        }
    ...
    private fun showAirportServices( apt: Cursor ) {
        ...
        val other = apt.getString(apt.getColumnIndexOrThrow(Airports.OTHER_SERVICES))
        if (other.isNotEmpty()) {
            services.addAll(DataUtils.decodeServices(other))
        }
        val storage = apt.getString(apt.getColumnIndexOrThrow(Airports.STORAGE_FACILITY))
        if (storage.isNotEmpty()) {
            services.addAll(DataUtils.decodeStorage(storage))
        }
        val bottOxygen = apt.getString(apt.getColumnIndexOrThrow(Airports.BOTTLED_O2_AVAILABLE))
        if (bottOxygen == "Y") {
            services.add("Bottled Oxygen")
        }
        val bulkOxygen = apt.getString(apt.getColumnIndexOrThrow(Airports.BULK_O2_AVAILABLE))
        if (bulkOxygen == "Y") {
    ...
    private fun showFaaServices(apt: Cursor) {
        ...
        val artccName = apt.getString(apt.getColumnIndexOrThrow(Airports.BOUNDARY_ARTCC_NAME))
        addRow(layout!!, "ARTCC", "$artccId ($artccName)")
        val siteNumber = apt.getString(apt.getColumnIndexOrThrow(Airports.SITE_NUMBER))
        val notamFacility = apt.getString(apt.getColumnIndexOrThrow(Airports.NOTAM_FACILITY_ID))
        ...
        val notamD = apt.getString(apt.getColumnIndexOrThrow(Airports.NOTAM_D_AVAILABLE))
